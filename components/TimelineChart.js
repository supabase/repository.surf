...
const TimelineChart = ({
  ...
  const [, setSelectedTimeFilter] = useState("allTime");
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      if (selectedTimeFilter !== "allTime") {
        const [selectedTimeFilterOption] = options.filter(
          (option) => option.key === selectedTimeFilter
        );
        const cutoffDate = new Date(
          currentTime - selectedTimeFilterOption.duration
        );
        ...
        series: [
          {
            value: (self, rawValue) => {
              ...
              return showOnlyDate
                ...
                : `${date.toLocaleDateString()} ${date.toLocaleString("en-US", {
                    hour: "numeric",
                    minute: "numeric",
                    hour12: true,
                  })}`;
