...
export const formatOrganizationIssueCount = (issueCounts) => {
  const overviewIssueCounts = []
  const formattedData = issueCounts.map(row => {
  ...
  return overviewIssueCounts.sort((a, b) => a.inserted_at - b.inserted_at)
}
...
export const retrieveStarGrowthToday = (starHistory) => {
  ...
  const datePrevious = formatDate(previousDate)
...
export const retrieveStarGrowthMonth = (starHistory) => {
  const dateString = `${new Date().toISOString().slice(0, 7)}-01`
...
export const retrieveUserOrganizations = async(accessToken) => {
  const { decrypted_token } = await postAndWait('/api/decrypt', { token: accessToken })
